...
import models.resnet as resnet
from models.att_resnet import ResNetAttBackbone
...
class SketchBYOL(tf.keras.Model):
    ...
    def get_encoder(self):
        ...
        bkbone = ResNetAttBackbone([3,4,6,3], [64,128, 256, 512], kernel_regularizer = tf.keras.regularizers.l2(self.WEIGHT_DECAY), num_att_blocks=1)
        x = bkbone(x)
        # Projection head.
    ...
    def fit_byol(self, data, epochs, ckp_dir):
        ...
        for epoch in range(epochs) :
            ...
            for step, batch in enumerate(data):                                     
                ...
                # sys.stdout.write(
                # '\rEpoch {}, batch {} - loss {:.4f} - elapsed time {}'.format(epoch+1, step+1, loss, elapsed_time_formatted))
